...
function animateCounter(element, target, duration = 2000) {
  ...
  const timer = setInterval(() => {
    ...
    if (current >= target) {
      ...
      element.textContent =
        Math.floor(current) + (target === parseInt(target) ? "+" : "");
    }
  }, 16);
}
  ...
  searchInput.addEventListener("input", function (e) {
    ...
    categories.forEach((category) => {
      const visibleItems = category.querySelectorAll(
        '.cert-item[style="display: flex;"], .cert-item:not([style*="display: none"])',
      );
...
console.log(
  "%c🎓 Darshil Shah Portfolio",
  "color: #2563eb; font-size: 20px; font-weight: bold;",
);
console.log(
  "%cView certifications and professional credentials",
